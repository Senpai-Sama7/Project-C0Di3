...
export class AuthService {
  ...
  async updateUser(userId: string, updates: Partial<User> & { password?: string }): Promise<User | null> {
    ...
    if (password) {
      ...
      if (typeof userUpdates.isActive !== 'boolean') {
  ...
  private migrateLegacyUserCredentials(
    ...
    if (!resolution) {
      const { hash, salt } = this.hashPassword(crypto.randomBytes(48).toString('hex'));
      ...
      if (warningKey && !this.legacyCredentialWarnings.has(warningKey)) {
        ...
        this.logger.error(
          `User ${user.username} has no recoverable credentials. Account disabled until an administrator resets the password.`
        );
      }

      return { user: disabledUser, persist: true };
    }
    ...
    if (warningKey && !this.legacyCredentialWarnings.has(warningKey)) {
      this.legacyCredentialWarnings.add(warningKey);
      this.logger.warn(
        `Migrated legacy credentials for user ${user.username}. Source=${source}. Account disabled until password reset.`
      );
    }
  ...
  private toPublicUser(user: StoredUser): User {
    ...
    return {
      ...publicUser,
      requiresPasswordReset: Boolean(publicUser.requiresPasswordReset),
